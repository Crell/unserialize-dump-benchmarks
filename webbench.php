...
foreach ($tests as $rule) {
    $subItems = $rule['s'];
    $recursion = $rule['r'];
    foreach ([1, 10] as $concurrency) {
        $totalItems = $subItems * 2 + $recursion + 1;
        shell_exec("SUB_ITEMS={$subItems} RECURSION={$recursion} php setup-serialized.php");

        shell_exec("ab -n {$webRuns} -c ${concurrency} -d -q  http://dumpbench.lndo.site/unserialize.php 2>&1 > temp.txt");
        $time = abfilter();
        print "unserialize(), $subItems, $recursion, $totalItems, $concurrency, $time \n";
        $results[$concurrency][$totalItems]['unserialize()'] = $time;
...
foreach ($results as $concurrency => $data) {
    ...
    foreach ($data as $totalItems => $data2) {
        print "$totalItems, {$data2['unserialize()']}, {$data2['require()']}\n";
    }
}
